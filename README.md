...
- **首頁**：今天是否已紀錄、連續紀錄天數、本月紀錄天數、近 7 天平均心情；已
  紀錄的話另外顯示「今天的你」（心情分數、主要情緒、感謝摘要）；每天固定顯示
  一題自我探索小問題（依日期決定、同一天不會變、隔天自動換新），可直接寫下
  回答並選擇讓 AI 給簡短回應與下一步建議。
...
- **月度 Insights**：每日心情折線圖、情緒出現次數長條圖、每週平均心情，加上
  ...
  變好還是變差）。
- **AI 情緒分析與建議（選用、免費）**：在「每日紀錄」「每週回顧」「Insights」
  頁面按下「AI 分析」，會用 Google Gemini API 的免費額度，針對當天／該週／
  該月的紀錄產生情緒分析、可能的壓力來源、具體建議與一句鼓勵的話。完全選用
  ——不設定 API Key 也不影響其他任何功能，仍會顯示免費的關鍵字式分析。
- **回顧語錄**：把所有寫過的「感謝的事」「做得不錯的事」依月份彙整成兩份清單
  （感謝清單 / 自我肯定紀錄），從 Insights 頁面進入。
- **時光信**：首頁會在剛好滿一個月、三個月、一年的日子，把你當時寫的「給自己
  的一句話」重新顯示出來，讓過去的自己跟現在的自己說話。
- **長期主題追蹤**：Insights 頁面另外顯示最近三個月內反覆出現的主題（門檻比
  單月更高），比單月回顧看得更長線。
